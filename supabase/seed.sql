...
-- Outlets (reference the brand by name; building_id is derived from location by
-- a trigger, so it isn't set here). Outlet `name` left null → defaults to brand.
insert into public.businesses (brand_id, location, address, data_source)
values
  ((select id from public.brands where name = 'Kopitiam food court stall'),
   ST_SetSRID(ST_MakePoint(103.7626, 1.3787), 4326)::geography, 'Bukit Panjang Plaza, 1 Jelebu Rd', 'manual'),
  ((select id from public.brands where name = 'Neighbourhood minimart'),
   ST_SetSRID(ST_MakePoint(103.7629, 1.3789), 4326)::geography, 'Bukit Panjang Plaza', 'manual'),
  ((select id from public.brands where name = 'International fast-food outlet'),
   ST_SetSRID(ST_MakePoint(103.7640, 1.3783), 4326)::geography, 'Hillion Mall, 17 Petir Rd', 'manual'),
  ((select id from public.brands where name = 'Indie bubble tea stall'),
   ST_SetSRID(ST_MakePoint(103.7638, 1.3780), 4326)::geography, 'Hillion Mall', 'manual'),
  ((select id from public.brands where name = 'Neighbourhood barber'),
   ST_SetSRID(ST_MakePoint(103.7624, 1.3785), 4326)::geography, 'Bukit Panjang Plaza', 'manual'),
  ((select id from public.brands where name = 'Local bakery chain'),
   ST_SetSRID(ST_MakePoint(103.7642, 1.3781), 4326)::geography, 'Hillion Mall', 'manual');
